					...
					/****Ends Captcha Code******/
							...
							else
							{
						            ...
						            $this->Base_model->update_record_by_id('tbl_applicant', $update_data, array('id'=> $insertid));
												$msg = "Registered successfully, Please login and check your mail for password.";
	...
	public function verify_phone()
	{
		...
		$this->form_validation->set_rules('email','Email','trim|required');
					...
					else
					{
							...
							if($checked=='1')
							{
									$res = array('status'=>'fails','msg' => 'Mobile no already verified', 'error' => 1);    
                	echo json_encode($res);
							} //ends if
                	...
                	echo json_encode($res);
			            ...
			            if($otp != '1')
			            {
			            	$update_data = array(
										...
										$insertid = $this->Base_model->update_record_by_id('tbl_verification', $update_data, array('email'=> $email));
