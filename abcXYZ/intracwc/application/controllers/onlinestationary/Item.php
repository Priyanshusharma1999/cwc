...
class Item extends CI_Controller {
	...
	public function index()
	{
		...
		$data['all_category'] = $this->Base_model->get_all_record_by_condition('category',array('service_type'=>'2','delete_status'=>'1'));

		$this->load->view('admin/header');
		$this->load->view('admin/sidebar-stationary');
		$this->load->view('admin/stationary_management/item/itemlist',$data);
					...
					} else
					 {
					  ...
					  $insert_data = array(
						...
						if($insertid)
						{
							...
							redirect('onlinestationary/item',$data);
						}
		...
		if(isset($_REQUEST['submit'])) 
		{
			...
			if($this->form_validation->run() === false) 
			{
				  ...
				  // $data['item_type'] = $this->Base_model->get_all_record_by_condition('item_type',array('delete_status'=>1));
				   ...
				   $data['all_subcategory'] = $this->Base_model->get_all_record_by_condition('sub_category',array('delete_status'=>1,'service_type'=>2,'subcat_id'=>$data['item_detail']->subcategory_id));

					$this->load->view('admin/header');
					$this->load->view('admin/sidebar-stationary');
					$this->load->view('admin/stationary_management/item/edititem',$data);
					$this->load->view('admin/footer');
	...
	public function viewitem(){
		   ...
		   $data['item_issuedet'] = $this->Base_model->get_all_record_by_condition('osr_requisition_item',array('item_id' => $uri,'approved_qty>'=>'0','service_type'=>'2'));
			
			$this->load->view('admin/header');
			$this->load->view('admin/sidebar-stationary');
			$this->load->view('admin/stationary_management/item/itemdetail',$data);
			$this->load->view('admin/footer');
	}
	...
	public function delete_item(){
			...
			redirect('onlinestationary/item');
	   ...
	   public function search_item()
	     {
	      ...
	      public function getallsubcategory(){
	      	...
	      	$allsubcategory =  $this->Base_model->get_all_record_by_condition('sub_category', array('category_id'=>$category_id,'delete_status'=>'1','service_type'=>'2'));
