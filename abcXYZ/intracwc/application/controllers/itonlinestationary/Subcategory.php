...
class Subcategory extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/itstationary_management/masterdata/subcategorylist',$data);
		   ...
		   $data['all_category'] = $this->Base_model->get_all_record_by_condition('category',array('delete_status'=>1,'service_type'=>1));
			...
			if($this->form_validation->run() === false) 
			{
				   $data['insertData'] = array(
					...
					$this->load->view('admin/header');
					$this->load->view('admin/sidebar-itstationary');
					$this->load->view('admin/itstationary_management/masterdata/addsubcategory',$data);
					$this->load->view('admin/footer');
					...
					} else
					 {
					  ...
					  $insert_data = array(
						...
						if($insertid)
						{
							...
							redirect('itonlinestationary/Subcategory');
						}
		...
		if(isset($_REQUEST['submit'])) 
		{
		   ...
		   $data['subcategory_detail'] = $this->Base_model->get_record_by_id('sub_category',array('subcat_id' => $uri));
			...
			{	    
					$this->load->view('admin/header');
					$this->load->view('admin/sidebar-itstationary');
					$this->load->view('admin/itstationary_management/masterdata/editsubcategory',$data);
					$this->load->view('admin/footer');
