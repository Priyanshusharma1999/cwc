...
class Solvecomplain extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/nonitcomplain_management/solvecomplainlist',$data);
	...
	public function resolvecomplain(){
		...
		if(isset($_REQUEST['submit'])) 
		{
			...
			{
				$data['complain_type'] = $this->Base_model->get_all_record_by_condition('category',array('status'=>'1','delete_status'=>'1','service_type'=>2));
				...
				$this->load->view('admin/header');
				$this->load->view('admin/sidebar-nonitasset');
				$this->load->view('admin/nonitcomplain_management/resolvecomplain',$data);
				$this->load->view('admin/footer');
			     ...
			     if($complain_detail->status=='Pending'){
					  ...
					  $update_data = array(
						...
						$data['complain_list'] = $this->Base_model->get_all_record_by_condition('complaint',array('delete_status'=>'1'));
							...
							redirect('nonitcomplaint/solvecomplain',$data);
						}
	...
	public function viewcomplain(){
			...
			$this->load->view('admin/header');
			$this->load->view('admin/sidebar-nonitasset');
			$this->load->view('admin/nonitcomplain_management/viewsolvecomplain',$data);
			$this->load->view('admin/footer');
