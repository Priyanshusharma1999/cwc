	 ...
	 <div class="page-wrapper">
			<div class="content container-fluid">
                <div class="row">
                       ...
                       <?php if($req_detail->is_miscallenous == '0'){?>
						...
						<input type='text' name='remarks[]' value="<?php echo $request->req_remark; ?>" hidden>
										...
										<td><?php echo $item_name->quantity_stock; ?>
											<input type='text'  value="<?php  echo $item_name->quantity_stock; ?>" id="remstock" hidden>
										</td>

										<td><?php echo $item_name->approved_stock; ?>
											<input type='text'  value="<?php echo $item_name->approved_stock; ?>" id="appstck" hidden>
										</td>
								...
								<?php $remaining_stock = $item_name->quantity_stock-$item_name->approved_stock; ?>		
					...
					<td class="qtycont">
		               ...
		               <input class="form-control qty" type="number" min="1" name="approveqty[]"  value="<?php echo $request->approved_qty; ?>">
					    <span class="text-danger"></span>
                         ...
                         <?php if($remaining_stock != '0'){?>	   
					       ...
					       <button type="submit" name="submit" class="btn btn-primary sv-btn" disabled>Save</button>

					   <?php } ?>

                           <a href="<?php echo base_url(); ?>onlinestationary/approverequisition" class="btn btn-danger">Cancel</a>						   
					 <?php echo form_close();?>		
                            </div>
                        </div>
                    </div>
		...
		<script>
			   ...
			   $('.qty').keyup(function(){
                   ...
                   var appqty = $(this).closest('tr').find('#appstck').val();

                   var remainingqty = parseInt(qnty1) - parseInt(appqty);

                   var qnty2 = $(this).closest('tr').find('#reqty').val();

                    qnty2 = parseInt(qnty2);

                    if(approvedqnty>remainingqty){
                      
                      msg = 'Approved quantity can not be greater than Remaining stock(Actual stock - approved stock).';
