...
class Base_model extends CI_Model
{
   ...
   public function search_pension($pension_status,$pension_type)
    {

        if($pension_status && $pension_type)
        {
          
            $this->db->select('*');
            $this->db->from('pensrecoinfo');
            $this->db->where(array('DELETES'=>'0','PENSION_TYPE' => $pension_type));
            ...
            if($pension_status && empty($pension_type))
             {
                $this->db->select('*');
                $this->db->from('pensrecoinfo');
                $this->db->where(array('DELETES'=>'0'));
             ...
             } else if($pension_type && empty($pension_status))
             {
   ...
   public function get_pension_record($pension_id)
    {
     ...
     public function get_expiry_employee($from_date,$to_date,$user_id)
    {
        ...
        if($user_role==1)
        {
             $query = $this->db->query("SELECT * FROM `pensrecoinfo` WHERE `DATE_RETIREMENT` BETWEEN '".$from_date."' AND '".$to_date."' AND `DELETES`='0'");
        }

        else
        {
          //  $query = $this->db->query("SELECT * FROM `pensrecoinfo` Where  '".$filter_date."' >= `DATE_RETIREMENT` AND `MODIFIEDBY_ID` =  '".$user_id."' AND `DELETES`='0' ");

            $user_id =  $this->session->userdata('applicant_user_id');

            $query = $this->db->query("SELECT * FROM `pensrecoinfo` WHERE `DATE_RETIREMENT` BETWEEN '".$from_date."' AND '".$to_date."' AND `MODIFIEDBY_ID` =  '".$user_id."' AND `DELETES`='0'");
