...
class Bill extends CI_Controller {
   ...
   function __construct()
	...
	public function index()
	{
		...
		$data['all_bill'] = $this->Base_model->get_all_record_by_condition('osr_bill_master',array('delete_status'=>0,'status'=>1,'service_type'=>2));
		...
		$this->load->view('admin/stationary_management/bill/billlist', $data);
		...
		if(isset($_REQUEST['submit'])) 
		{
		   ...
		   $data['insertData'] = array(
			...
			$data['all_items'] = $this->Base_model->get_all_record_by_condition('osr_item_master',array('status'=>1,'delete_status'=>0,'service_type'=>2));
				  ...
				  $this->load->view('admin/header');
				  $this->load->view('admin/sidebar-stationary');
				  $this->load->view('admin/stationary_management/bill/entrybill',$data);
				  $this->load->view('admin/footer');
				  ...
				  $data['insertData'] = array(
					...
					'remark'		  => xss_clean(strip_tags($this->input->post('remark')))
					...
					} else
					 {
						...
						if($_FILES['bill_copy']['name'])
				         {
			                  $configg = array(
			                             'upload_path' => "./uploads/bill_copy/",
			                             'allowed_types' => "jpg|png|jpeg|pdf",
			                             'overwrite' => TRUE,
			                             'max_size' => "4096000", 
			                             'file_name' => $pic_name.$_FILES["bill_copy"]['name'],
			                             );              
			                   $this->load->library('upload', $configg);
			                   $this->upload->initialize($configg);
			                   $img_namee= $_FILES['bill_copy']['name'];
			                  ...
			                  {  
			                     ...
			                     $file_path ='uploads/bill_copy/'.$img_namee;
			                  }
				...
				foreach($item as $items){
					...
					$i++;
							...
							if($insertid)
							{
								...
								redirect('onlinestationary/bill',$data);
							}
	...
	public function editbill(){
		...
		if(isset($_REQUEST['submit'])) 
		{
			...
			$update_master = array(
				...
				'bill_date' 	        => $billdate,
				'vendor_name' 	        => $vendorname,
				'bill_copy'			    => !empty($img_namee)?$img_namee:$billdetail->bill_copy,
				'vendor_address' 	    => !empty($address)?$address:NULL,
				'vendor_email' 	        => !empty($email)?$email:NULL,
				'vendor_contact_no' 	=> $contact,
				'order_no' 	            => !empty($orderno)?$orderno:NULL,
	  ...
	  foreach($olditems as $items){
		...
		$this->Base_model->delete_record_by_id('osr_bill_item',array('bill_master_id'=>$uri));
			 ...
			 $data['bill_detail'] = $this->Base_model->get_record_by_id('osr_bill_master',array('bill_master_id' => $uri));
				...
				$this->load->view('admin/header');
				$this->load->view('admin/sidebar-stationary');
				$this->load->view('admin/stationary_management/bill/editbill',$data);
				$this->load->view('admin/footer');
	...
	public function viewbill()
	{
		...
		$this->load->view('admin/header');
		$this->load->view('admin/sidebar-stationary');
		$this->load->view('admin/stationary_management/bill/viewbill',$data);
		$this->load->view('admin/footer');
