...
class Osradmin extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('admin/itstationary_management/osradmin/physicalissuelist',$data);
	...
	public function proxylist()
	{
		...
		$this->load->view('admin/header');
		$this->load->view('admin/sidebar-itstationary',$roledata);
		$this->load->view('admin/itstationary_management/osradmin/proxyissuelist',$data);
		$this->load->view('admin/footer');
			...
			if($this->form_validation->run() === false) 
			{
				...
				$data['all_items'] = $this->Base_model->get_all_record_by_condition('osr_item_master',array('status'=>1,'delete_status'=>0,'service_type'=>1));
				   ...
				   $data['insertData'] = array(
					...
					$this->load->view('admin/header');
					$this->load->view('admin/sidebar-itstationary',$roledata);
					$this->load->view('admin/itstationary_management/osradmin/addproxy',$data);
					$this->load->view('admin/footer');
				   ...
				   if($reqid){
					   ...
					   $insertid = $this->Base_model->insert_one_row('osr_requisition_item', 
						...
						if($insertid)
						{
							...
							redirect('itonlinestationary/osradmin/proxylist',$data);
						}
   ...
   // 	echo 'aaaaaaaaaaaaaaaaaaaaaa'; exit;
		     ...
		     if($this->form_validation->run() === false) 
		      {
				...
				$data['all_request'] = $this->Base_model->get_all_record_by_condition('osr_requisition_item',array('req_id'=>$uri));
					...
					$this->load->view('admin/header');
					$this->load->view('admin/sidebar-itstationary',$roledata);
					$this->load->view('admin/itstationary_management/osradmin/issue',$data);
					$this->load->view('admin/footer');
		  ...
		  if($finalid)
			{
                    ...
                    $this->email->message('Product has issued to '.$mail->display_name);
                    $this->email->send();
					
					redirect('itonlinestationary/osradmin',$data);
			}
		...
		if(isset($_REQUEST['submit'])) 
		{
			...
			if($this->form_validation->run() === false) 
			{
			   ...
			   $uri = $this->uri->segment('4'); 
				...
				$this->load->view('admin/header');
				$this->load->view('admin/sidebar-itstationary',$roledata);
				$this->load->view('admin/itstationary_management/requestapprove/approverequest',$data);
				$this->load->view('admin/footer');
				...
				$this->load->view('admin/header');
				$this->load->view('admin/sidebar-itstationary',$roledata);
				$this->load->view('admin/itstationary_management/osradmin/editproxy',$data);
				$this->load->view('admin/footer');
	...
	public function viewrequest()
	{
		...
		$this->load->view('admin/header');
		$this->load->view('admin/sidebar-itstationary',$roledata);
		$this->load->view('admin/itstationary_management/osradmin/viewrequest',$data);
		$this->load->view('admin/footer');
